...
if rank == 0:
    ...
    # (4) 시작 분자 및 상위 8개 분자 구조 시각화 이미지를 생성하고 저장한다.
    from rdkit.Chem import Draw
    start_mol = Chem.MolFromSmiles(START_SMI)
    start_score = history[START_SMI]
    top_8 = df_history.head(8)
    top_mols = []
    top_legends = []
    # 시작 분자를 첫 번째 칸에 배치한다.
    if start_mol:
        top_mols.append(start_mol)
        top_legends.append(f"[START] Score: {start_score:.4f}")
    for idx, row in top_8.iterrows():
        mol = Chem.MolFromSmiles(row['SMILES'])
        if mol:
            top_mols.append(mol)
            top_legends.append(f"Rank {idx+1} (Score: {row['Score']:.4f})")
    top_mols = top_mols[:8]  # 시작 분자 1개 + 상위 7개 = 최대 8개
    top_legends = top_legends[:8]
