...
all_files = sorted(glob.glob('./zinc_db/*.txt')[:10])
combined_list = []
 
# 7. 컷오프 필터 적용 전 원본 후보군 저장용 CSV 파일 초기화 수행
calculated_csv_file = 'zinc_candidates_all_calculated_test.csv'
if rank == 0:
...
if rank == 0:
    ...
    plt.savefig('similarity_distribution_comparison_raw_test.png', dpi=300)
    ...
    plt.legend(fontsize=10, loc='upper right')
    plt.savefig('similarity_distribution_comparison_cutoff_test.png', dpi=300)
    ...
    neg_before_file = 'negative_candidates_before_sampling_test.csv'
    ...
    if sample_size > 0:
        ...
        final_neg.to_csv('negative_agro_vs_zinc_test.csv', index=False)
