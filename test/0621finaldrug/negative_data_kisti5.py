...
prefix = "testf_" if test == 1 else "final_"
raw_img_file = os.path.join(BASE_DIR, f"{prefix}distribution_comparison_raw.png")
cutoff_img_file = os.path.join(BASE_DIR, f"{prefix}distribution_comparison_cutoff.png")
final_img_file = os.path.join(BASE_DIR, f"{prefix}distribution_comparison.png")
neg_before_file = os.path.join(BASE_DIR, f"{prefix}negative_candidates_before_sampling.csv")
final_neg_file = os.path.join(BASE_DIR, f"{prefix}negative_agro_vs_zinc.csv")
metadata_file = os.path.join(BASE_DIR, f"{prefix}negative_metadata.txt")
...
if size < 2:
    if rank == 0:
        print("ERROR: MPI 프로세스가 최소 2개 필요합니다. 예: mpiexec -n 5 python negative_data_kisti5.py")
    raise SystemExit(1)
...
if rank == 0:
    ...
    pos_df = pd.read_csv(os.path.join(BASE_DIR, 'PubChem_Agrochemical.csv'))
...
# 분산 연산 대상이 될 대용량 ZINC 데이터베이스 파일들을 정해진 디렉토리 내부에서 무작위 탐색 (300배수 채울 때까지)
if rank == 0:
    all_files_raw = glob.glob(os.path.join(BASE_DIR, 'zinc_db', '*.txt'))
    if test == 1:
        all_files_raw = sorted(all_files_raw, key=os.path.getsize)[:10]

    pos_count = len(pos_smiles)
    target_count = pos_count * TARGET_MULTIPLIER
    
    random.seed()
    random.shuffle(all_files_raw)
    
    if PROCESS_ALL_ZINC:
        selected_files = all_files_raw
    else:
        selected_files = []
        ...
        for fpath in all_files_raw:
            ...
            try:
                ...
                current_molecule_count += lines
            except Exception:
                pass
                
            if current_molecule_count >= target_count:
                break
else:
    selected_files = None

# 마스터가 선별한 파일 리스트를 워커들에게 브로드캐스트
all_files = comm.bcast(selected_files, root=0)
combined_list = []
...
# 워커 노드들이 마스터에게 첫 신호를 보내어 일감을 요청하고 대기하는 루프 개시
    ...
    while True:
        ...
        try:
            for chunk in pd.read_csv(file_path, sep='\t', usecols=['smiles', 'zinc_id', 'mwt', 'logp'], chunksize=CHUNKSIZE):
                ...
                for s in smi_arr:
                    ...
                    chunk_sims.append(max_sim)
                    local_count += 1

                if chunk_sims:
                    chunk_counts, _ = np.histogram(chunk_sims, bins=bins)
                    local_counts += chunk_counts
...
if rank == 0:
    ...
    while finished_workers < size - 1:
        status = MPI.Status()
        local_res = comm.recv(source=MPI.ANY_SOURCE, tag=MPI.ANY_TAG, status=status)
        worker = status.Get_source()

        if status.Get_tag() == RESULT_BATCH_TAG:
            if local_res:
                ...
                if len(write_buffer) >= RESULT_BATCH_SIZE:
            ...
            for chunk in pd.read_csv(file_path, sep="\t", usecols=["smiles", "zinc_id", "mwt", "logp"], chunksize=CHUNKSIZE):
                ...
                for s, z_id, m, x in zip(smi_arr, id_arr, mw_arr, logp_arr):
                    ...
                    if max_sim <= optimal_cutoff:
                        ...
                        if len(local_negatives) >= RESULT_BATCH_SIZE:
                            comm.send(local_negatives, dest=0, tag=RESULT_BATCH_TAG)
                            local_negatives = []
...
if rank == 0:
    ...
    if sample_size > 0:
        ...
        with open(neg_before_file, "r", encoding="utf-8", newline="") as f:
            ...
            for idx, row in enumerate(reader, 1):
                if idx in selected_rows:
                    final_neg_list.append({
                        ...
                        "max_similarity_to_positive": float(row["max_similarity_to_positive"])
                    })
    ...
    # 메타데이터 파일 작성 및 저장
    metadata_content = f"""[Task 1 Execution Metadata - KISTI MPI (Random Target Subsampling Version)]
