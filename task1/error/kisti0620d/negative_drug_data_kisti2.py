...
final_neg_file = f"{prefix}negative_agro_vs_zinc.csv"
...
# 0번 마스터 프로세스가 로컬 디스크로부터 기준점이 될 양성(약) 데이터셋을 로드하고 필수 변수명을 표준화 처리
if rank == 0:
...
if rank == 0:
    all_files_raw = glob.glob('./zinc_db/*.txt')
    pos_count = len(pos_smiles)
    target_count = pos_count * 300
...
Total Drug Positives: {len(pos_df)}
